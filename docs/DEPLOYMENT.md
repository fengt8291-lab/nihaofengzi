...
- `/robots.txt`、`/sitemap-index.xml`、`/favicon.svg` 必须返回 HTTP 200；
- 请求统一添加基于本次 short SHA 的 `deploy` 查询参数，降低缓存误判；
- 所有生产验证请求统一使用 10 秒连接时限、60 秒总时限、3 次重试和 2 秒重试间隔，并保持 `--fail --silent --show-error`；
...
- sitemap index 响应必须包含 `<sitemapindex` 标识；
